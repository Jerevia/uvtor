...
from uvtor.db.migrations import LOGGER, MigrateHistory
from uvtor.db.migrations.auto import diff_many, NEWLINE
from uvtor.db.migrations.compat import string_types, exec_in
from uvtor.db.migrations.migrator import Migrator


CLEAN_RE = re.compile(r'\s+$', re.M)
CURDIR = os.getcwd()
DEFAULT_MIGRATE_DIR = os.path.join(CURDIR, 'models/migrations')


def VOID(m, d): return None  # noqa
        ...
        if auto:
            try:
                ...
                return self.logger.error(
                    "Can't import models module: %s", auto)
        ...
        try:
            ...
            with self.database.transaction():
                if not downgrade:
                    ...
                    migrator.run()
                    self.model.delete().\
                        where(self.model.name == name).execute()
...
class Router(BaseRouter):
    ...
    def todo(self):
        ...
        return sorted(f[:-3] for f in os.listdir(self.migrate_dir)
