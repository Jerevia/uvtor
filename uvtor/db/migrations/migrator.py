...
from uvtor.db.migrations import LOGGER
...
class Migrator(object):
    ...
    def drop_columns(self, model, *names, **kwargs):
        ...
        for field in fields:
            ...
            if field.unique:
                compiler = self.database.compiler()
                index_name = compiler.index_name(
                    model._meta.db_table, (field.db_column,))
                self.ops.append(
                    self.migrator.drop_index(
                        model._meta.db_table,
    ...
    def drop_index(self, model, *columns):
        ...
        for col in columns:
            ...
            if isinstance(field, pw.ForeignKeyField):
                col = col + '_id'
            columns_.append(col)
        index_name = self.migrator.database.compiler(
        ).index_name(model._meta.db_table, columns_)
        model._meta.indexes = [
            (cols, _) for (
    ...
    def add_default(self, model, name, default):
        ...
        self.ops.append(
            self.migrator.apply_default(
                model._meta.db_table, name, field))
        return model
